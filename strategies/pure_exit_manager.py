...
class PureExitManager:
    """
    纯净版平仓管理器 (Lifecycle Manager) - 增强版 V2.1
    1. 使用 initial_entry_time 锚定止盈计算
    ...
    def _calculate_target_price(self, minutes_to_close: float, position: Position, 
        ...
        if 20 < minutes_to_close <= 240:
            # 【关键逻辑】使用 initial_entry_time 计算衰减进度
            # 这样即使部分成交，时间锚点也不会变，止盈线平滑下降
